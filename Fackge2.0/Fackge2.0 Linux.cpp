...
void SendCommand(string command, int mode, int line, int end) {
	if(command == "help") {
		...
		for (int i = 0; i < CommandNumber ; i++) {
			cout<<CommandManager[i]<<":\n 用法:"<<CommandDPT[i]<<endl;
		}
		return;
	} else if(command == "fload") {
		thread thrd_1(loadfile);
		//加载插件移交多线程 
...
void fileprint(string command, int line) {
	...
	if(CanLoad==true) {
		if (line==1&&command!="fload{") {
			...
			int end = command.length();
			string str1 = command.substr(14,end);
			//第一次,去掉publucver
			string pv;
			...
			int end = command.length();
			string str1 = command.substr(3,end);
			//以后都要注意了呃呃呃 
			istringstream is(str1);
			is>>str[0]>>str[1]>>str[2];
